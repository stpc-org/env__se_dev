// Decompiled with JetBrains decompiler
// Type: VRageMath.Spatial.MyVector3Grid`1
// Assembly: VRage.Math, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: B60E95EA-339C-4CC9-9413-1B8A10CB206E
// Assembly location: D:\Files\library_development\lib_se\VRage.Math.dll

using System.Collections.Generic;
using System.Diagnostics;
using VRage.Library.Collections;

namespace VRageMath.Spatial
{
  public class MyVector3Grid<T>
  {
    private float m_cellSize;
    private float m_divisor;
    private int m_nextFreeEntry;
    private int m_count;
    private MyList<MyVector3Grid<T>.Entry> m_storage;
    private Dictionary<Vector3I, int> m_bins;
    private IEqualityComparer<T> m_equalityComparer;

    public int Count => this.m_count;

    public int InvalidIndex => -1;

    public MyVector3Grid(float cellSize)
      : this(cellSize, (IEqualityComparer<T>) EqualityComparer<T>.Default)
    {
    }

    public MyVector3Grid(float cellSize, IEqualityComparer<T> comparer)
    {
      this.m_cellSize = cellSize;
      this.m_divisor = 1f / this.m_cellSize;
      this.m_storage = new MyList<MyVector3Grid<T>.Entry>();
      this.m_bins = new Dictionary<Vector3I, int>();
      this.m_equalityComparer = comparer;
      this.Clear();
    }

    public void Clear()
    {
      this.m_nextFreeEntry = 0;
      this.m_count = 0;
      this.m_storage.Clear();
      this.m_bins.Clear();
    }

    public void ClearFast()
    {
      this.m_nextFreeEntry = 0;
      this.m_count = 0;
      this.m_storage.ClearFast();
      this.m_bins.Clear();
    }

    public void AddPoint(ref Vector3 point, T data)
    {
      Vector3I binIndex = this.GetBinIndex(ref point);
      int index;
      if (!this.m_bins.TryGetValue(binIndex, out index))
      {
        int num = this.AddNewEntry(ref point, data);
        this.m_bins.Add(binIndex, num);
      }
      else
      {
        MyVector3Grid<T>.Entry entry = this.m_storage[index];
        for (int nextEntry = entry.NextEntry; nextEntry != this.InvalidIndex; nextEntry = entry.NextEntry)
        {
          index = nextEntry;
          entry = this.m_storage[index];
        }
        int num = this.AddNewEntry(ref point, data);
        entry.NextEntry = num;
        this.m_storage[index] = entry;
      }
    }

    public void RemovePoint(ref Vector3 point)
    {
      Vector3I binIndex = this.GetBinIndex(ref point);
      int num1;
      if (!this.m_bins.TryGetValue(binIndex, out num1))
        return;
      int index = this.InvalidIndex;
      int num2 = num1;
      MyVector3Grid<T>.Entry entry1 = new MyVector3Grid<T>.Entry();
      while (num1 != this.InvalidIndex)
      {
        MyVector3Grid<T>.Entry entry2 = this.m_storage[num1];
        if (entry2.Point == point)
        {
          int num3 = this.RemoveEntry(num1);
          if (num2 == num1)
          {
            num2 = num3;
          }
          else
          {
            entry1.NextEntry = num3;
            this.m_storage[index] = entry1;
          }
          num1 = num3;
        }
        else
        {
          index = num1;
          entry1 = entry2;
          num1 = entry2.NextEntry;
        }
      }
      if (num2 == this.InvalidIndex)
        this.m_bins.Remove(binIndex);
      else
        this.m_bins[binIndex] = num2;
    }

    public void MovePoint(int index, ref Vector3 newPosition)
    {
      MyVector3Grid<T>.Entry entry1 = this.m_storage[index];
      Vector3I binIndex1 = this.GetBinIndex(this.m_storage[index].Point);
      Vector3I binIndex2 = this.GetBinIndex(ref newPosition);
      if (binIndex1 == binIndex2)
      {
        entry1.Point = newPosition;
        this.m_storage[index] = entry1;
      }
      else
      {
        int bin = this.m_bins[binIndex1];
        int nextEntry;
        if (index == bin)
        {
          int num = this.RemoveEntry(index);
          if (num == this.InvalidIndex)
            this.m_bins.Remove(binIndex1);
          else
            this.m_bins[binIndex1] = num;
        }
        else
        {
          for (int index1 = bin; index1 != this.InvalidIndex; index1 = nextEntry)
          {
            MyVector3Grid<T>.Entry entry2 = this.m_storage[index1];
            nextEntry = entry2.NextEntry;
            if (nextEntry == index)
            {
              entry2.NextEntry = this.RemoveEntry(index);
              this.m_storage[index1] = entry2;
              break;
            }
          }
        }
        this.AddPoint(ref newPosition, entry1.Data);
      }
    }

    public int FindPointIndex(ref Vector3 point, T data)
    {
      Vector3I binIndex = this.GetBinIndex(ref point);
      int index = this.InvalidIndex;
      MyVector3Grid<T>.Entry entry;
      for (this.m_bins.TryGetValue(binIndex, out index); index != this.InvalidIndex; index = entry.NextEntry)
      {
        entry = this.m_storage[index];
        if (entry.Point == point && this.m_equalityComparer.Equals(entry.Data, data))
          return index;
      }
      return index;
    }

    public T GetData(int index) => this.m_storage[index].Data;

    public Vector3 GetPoint(int index) => this.m_storage[index].Point;

    public MyVector3Grid<T>.SphereQuery QueryPointsSphere(
      ref Vector3 point,
      float dist)
    {
      return new MyVector3Grid<T>.SphereQuery(this, ref point, dist);
    }

    public void RemoveTwo(
      ref MyVector3Grid<T>.SphereQuery en0,
      ref MyVector3Grid<T>.SphereQuery en1)
    {
      if (en0.CurrentBin == en1.CurrentBin)
      {
        if (en0.StorageIndex == en1.PreviousIndex)
        {
          en1.RemoveCurrent();
          en0.RemoveCurrent();
          en1 = en0;
        }
        else if (en1.StorageIndex == en0.PreviousIndex)
        {
          en0.RemoveCurrent();
          en1.RemoveCurrent();
          en0 = en1;
        }
        else if (en0.StorageIndex == en1.StorageIndex)
        {
          en0.RemoveCurrent();
          en1 = en0;
        }
        else
        {
          en0.RemoveCurrent();
          en1.RemoveCurrent();
        }
      }
      else
      {
        en0.RemoveCurrent();
        en1.RemoveCurrent();
      }
    }

    public Dictionary<Vector3I, int>.Enumerator EnumerateBins() => this.m_bins.GetEnumerator();

    public int GetNextBinIndex(int currentIndex) => currentIndex == this.InvalidIndex ? this.InvalidIndex : this.m_storage[currentIndex].NextEntry;

    public void GetLocalBinBB(ref Vector3I binPosition, out BoundingBoxD output)
    {
      output.Min = (Vector3D) (binPosition * this.m_cellSize);
      output.Max = output.Min + new Vector3(this.m_cellSize);
    }

    public void CollectStorage(int startingIndex, ref List<T> output)
    {
      output.Clear();
      MyVector3Grid<T>.Entry entry = this.m_storage[startingIndex];
      output.Add(entry.Data);
      while (entry.NextEntry != this.InvalidIndex)
      {
        entry = this.m_storage[entry.NextEntry];
        output.Add(entry.Data);
      }
    }

    public void CollectEntireStorage(List<T> output)
    {
      output.Clear();
      using (Dictionary<Vector3I, int>.Enumerator enumerator = this.m_bins.GetEnumerator())
      {
label_4:
        if (!enumerator.MoveNext())
          return;
        int nextEntry = enumerator.Current.Value;
        do
        {
          MyVector3Grid<T>.Entry entry = this.m_storage[nextEntry];
          output.Add(entry.Data);
          nextEntry = entry.NextEntry;
        }
        while (nextEntry != this.InvalidIndex);
        goto label_4;
      }
    }

    private Vector3I GetBinIndex(ref Vector3 point) => Vector3I.Floor(point * this.m_divisor);

    private Vector3I GetBinIndex(Vector3 point) => this.GetBinIndex(ref point);

    private int AddNewEntry(ref Vector3 point, T data)
    {
      ++this.m_count;
      if (this.m_nextFreeEntry == this.m_storage.Count)
      {
        this.m_storage.Add(new MyVector3Grid<T>.Entry()
        {
          Point = point,
          Data = data,
          NextEntry = this.InvalidIndex
        });
        return this.m_nextFreeEntry++;
      }
      if ((long) (uint) this.m_nextFreeEntry > (long) this.m_storage.Count)
        return -1;
      int nextFreeEntry = this.m_nextFreeEntry;
      this.m_nextFreeEntry = this.m_storage[this.m_nextFreeEntry].NextEntry;
      this.m_storage[nextFreeEntry] = new MyVector3Grid<T>.Entry()
      {
        Point = point,
        Data = data,
        NextEntry = this.InvalidIndex
      };
      return nextFreeEntry;
    }

    private int RemoveEntry(int toRemove)
    {
      --this.m_count;
      MyVector3Grid<T>.Entry entry = this.m_storage[toRemove];
      int nextEntry = entry.NextEntry;
      entry.NextEntry = this.m_nextFreeEntry;
      entry.Data = default (T);
      this.m_nextFreeEntry = toRemove;
      this.m_storage[toRemove] = entry;
      return nextEntry;
    }

    [Conditional("DEBUG")]
    private void CheckIndexIsValid(int index)
    {
      int index1 = this.m_nextFreeEntry;
      while (index1 != this.InvalidIndex && index1 != this.m_storage.Count)
        index1 = this.m_storage[index1].NextEntry;
    }

    private struct Entry
    {
      public Vector3 Point;
      public T Data;
      public int NextEntry;

      public override string ToString() => this.Point.ToString() + ", -> " + this.NextEntry.ToString() + ", Data: " + this.Data.ToString();
    }

    public struct SphereQuery
    {
      private MyVector3Grid<T> m_parent;
      private Vector3 m_point;
      private float m_distSq;
      private int m_previousIndex;
      private int m_storageIndex;
      private Vector3I_RangeIterator m_rangeIterator;

      public SphereQuery(MyVector3Grid<T> parent, ref Vector3 point, float dist)
      {
        this.m_parent = parent;
        this.m_point = point;
        this.m_distSq = dist * dist;
        Vector3 vector3 = new Vector3(dist);
        Vector3I binIndex1 = this.m_parent.GetBinIndex(point - vector3);
        Vector3I binIndex2 = this.m_parent.GetBinIndex(point + vector3);
        this.m_rangeIterator = new Vector3I_RangeIterator(ref binIndex1, ref binIndex2);
        this.m_previousIndex = -1;
        this.m_storageIndex = -1;
      }

      public T Current => this.m_parent.m_storage[this.m_storageIndex].Data;

      public Vector3I CurrentBin => this.m_rangeIterator.Current;

      public int PreviousIndex => this.m_previousIndex;

      public int StorageIndex => this.m_storageIndex;

      public bool RemoveCurrent()
      {
        this.m_storageIndex = this.m_parent.RemoveEntry(this.m_storageIndex);
        if (this.m_previousIndex == -1)
        {
          if (this.m_storageIndex == -1)
            this.m_parent.m_bins.Remove(this.m_rangeIterator.Current);
          else
            this.m_parent.m_bins[this.m_rangeIterator.Current] = this.m_storageIndex;
        }
        else
        {
          MyVector3Grid<T>.Entry entry = this.m_parent.m_storage[this.m_previousIndex];
          entry.NextEntry = this.m_storageIndex;
          this.m_parent.m_storage[this.m_previousIndex] = entry;
        }
        return this.FindFirstAcceptableEntry();
      }

      public bool MoveNext()
      {
        if (this.m_storageIndex == -1)
        {
          if (!this.FindNextNonemptyBin())
            return false;
        }
        else
        {
          this.m_previousIndex = this.m_storageIndex;
          this.m_storageIndex = this.m_parent.m_storage[this.m_storageIndex].NextEntry;
        }
        return this.FindFirstAcceptableEntry();
      }

      private bool FindFirstAcceptableEntry()
      {
        do
        {
          MyVector3Grid<T>.Entry entry;
          for (; this.m_storageIndex != -1; this.m_storageIndex = entry.NextEntry)
          {
            entry = this.m_parent.m_storage[this.m_storageIndex];
            if ((double) (entry.Point - this.m_point).LengthSquared() < (double) this.m_distSq)
              return true;
            this.m_previousIndex = this.m_storageIndex;
          }
          this.m_rangeIterator.MoveNext();
        }
        while (this.FindNextNonemptyBin());
        return false;
      }

      private bool FindNextNonemptyBin()
      {
        this.m_previousIndex = -1;
        if (!this.m_rangeIterator.IsValid())
          return false;
        Vector3I next = this.m_rangeIterator.Current;
        while (!this.m_parent.m_bins.TryGetValue(next, out this.m_storageIndex))
        {
          this.m_rangeIterator.GetNext(out next);
          if (!this.m_rangeIterator.IsValid())
            return false;
        }
        return true;
      }
    }
  }
}
