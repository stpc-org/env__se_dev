...
namespace Sandbox.Game.GameSystems.TextSurfaceScripts
{
  ...
  public class MyTSSEnergyHydrogen : MyTSSCommon
  {
      ...
      : base((Sandbox.ModAPI.Ingame.IMyTextSurface) surface, (VRage.Game.ModAPI.Ingame.IMyCubeBlock) block, size)
    {
      ...
      if (this.m_grid == null)
        return;
      this.m_resourceDistributor = (MyResourceDistributorComponent) this.m_grid.GridSystems.ResourceDistributor;
    ...
    public override void Run()
    {
      ...
      using (MySpriteDrawFrame frame = this.m_surface.DrawFrame())
      {
        ...
        frame.Add(sprite1);
        MySprite sprite2 = new MySprite(data: MyTSSEnergyHydrogen.ENERGY_ICON, color: new Color?(this.m_foregroundColor))
        {
        ...
        frame.Add(sprite2);
        this.AddProgressBar(frame, new Vector2(this.m_halfSize.X - num1, this.m_firstLine), new Vector2(x, vector2_1.Y * 0.4f), ratio1, barBgColor, this.m_foregroundColor);
        float num3 = 0.0f;
        ...
        frame.Add(sprite3);
        mySprite = new MySprite(data: MyTSSEnergyHydrogen.HYDROGEN_ICON, color: new Color?(this.m_foregroundColor));
        ...
        frame.Add(sprite4);
        this.AddProgressBar(frame, new Vector2(this.m_halfSize.X - num1, this.m_secondLine), new Vector2(x, vector2_1.Y * 0.4f), ratio2, barBgColor, this.m_foregroundColor);
    ...
    private void Recalculate()
    {
      ...
      {
        if (conveyorEndpointBlock is Sandbox.Game.Entities.Interfaces.IMyGasTank myGasTank && myGasTank.IsResourceStorage(MyResourceDistributorComponent.HydrogenId))
        {
    ...
    {
      if (!(myCubeBlock is Sandbox.Game.Entities.Interfaces.IMyGasTank myGasTank) || !myGasTank.IsResourceStorage(MyResourceDistributorComponent.HydrogenId))
        return;
      this.m_maxHydrogen -= myGasTank.GasCapacity;
